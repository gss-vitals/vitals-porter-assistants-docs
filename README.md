...
* FLAG: 你可以混合這些FLAG，例如：當設定為FLAG為5則表示可以檢視及更新文件夾
    ...
    * 你能設定多個權限，只需將這些權限以","分隔開，例如：admin:0=63,{ay7b7c21-fbe5-459a-af75-3b73b3fab283}:1=1
...
* FLAG: 你可以混合這些FLAG,例如，當設定為FLAG為5則表示可以檢視及列印文件
    ...
    * 檢視+更新+下載+預覽
        * 1+2+16+32=51
    * 你能設定多個權限，只需將這些權限以","分隔開，例如：admin:0=127,{ay7b7c21-fbe5-459a-af75-3b73b3fab283}:1=1
...
* 在**文件夾**和**分類夾**工作表：
    ...
    * 以`/`為開頭，不加也可以 (加`/`是好習慣)
    * 特殊情境：
        * 路徑欄位中包含不存在的資料夾或分類夾：匯出時也會自動產生相關的中介資料，其設定會繼承上一層目錄的設定
        * 若要表達最上層根目錄，路徑欄位請填 **`../`**
        * 路徑或名稱有包含`/`：請改以 **`%2F`** 表示
        * 路徑或名稱有包含`,`，請改以 **`%2C`** 表示
        * 路徑或名稱不可包含以下特殊字元：`[<>:|"?*\\t]`
...
## 知識文件填寫補充說明
![editDocument.png](/img/editDocument.png)
* 第一區塊的附檔路徑，需讓後續的匯入程式也可以讀取得到
* 操作建議順序：
